...
Compactモードのウインドウサイズは、表示しているパネル構成ごとに記憶されます。また、パネル構成、表示モード、Compact状態、ウインドウサイズを、名前付きのCustomレイアウトとして複数保存できます。

`Custom > Edit saved layouts...`では、保存済みレイアウトの即時プレビュー、名前変更、削除、保存内容の確認、`Up`／`Down`による表示順変更ができます。
...
ZIPにはオフラインで参照できる英語版／日本語版READMEも同梱されています。
...
`Custom`サブメニューでは、現在の画面構成に名前を付けて保存したり、保存済みレイアウトを呼び出したりできます。複数のレイアウトを保存でき、`Edit saved layouts...`で選択すると動作中のアプリへ即時反映されます。
...
選択した表示、色、パネル構成、Compact時のウインドウサイズ、Customレイアウト、ウインドウモードなどは自動的に保存され、次回起動時に復元されます。
